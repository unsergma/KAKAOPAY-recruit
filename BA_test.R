  ...
  with(hourtable, plot(Var1,SRate,type="l"))
  ...
  ## data
  depts <- dutchpay %>% filter(position=="deptor") %>% group_by(remittance_claim_send_id,status) %>% summarise(total=n()) %>%
                        group_by(remittance_claim_send_id) %>% mutate(percent=total/sum(total))
